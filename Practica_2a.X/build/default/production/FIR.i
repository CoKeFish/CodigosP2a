...
# 1 "./15hzorden11 12bL 10bL.h" 1
# 38 "./15hzorden11 12bL 10bL.h"
...
unsigned long FIR(const int *coeffs, int input) {
    ...
    for (int i = 0; i < 12; i++) {
        output += (unsigned long) coeffs[i] * (unsigned long) buffer[(index - i) & (32 - 1)];
    }
...
unsigned long FIR_p(const int32_t *coeffs, int input) {
    ...
    static int *endBuffer = buffer + (32 - 1);

    static int *head = buffer;
    static int *tail = buffer + ((-12) & (32 - 1)) ;


    static int *head_i = buffer;
    static int *tail_i = buffer + ((-12 + 1) & (32 - 1));


    static int indexHead = 0;
    static int indexTail = ((-12 + 1) & (32 - 1));
    ...
    *(head) = input;
    ...
    indexHead = (indexHead + 1) & (32 - 1);
    indexTail = (indexTail + 1) & (32 - 1);
